...
<body>
    <section class="container">
        ...
        <div class="container-right">
            ...
            <div class="isian">
                <form action="" method="POST">
                    ...
                    if(isset($_POST['submit'])){
                        ...
                        $query = mysqli_query($conn, "SELECT * FROM pengunjung WHERE email='".$email."' AND password='".md5($pass)."' AND status=0");
                    
                        if(mysqli_num_rows($query) > 0){
                            $data = mysqli_fetch_object($query);
                            $_SESSION['status_login'] = true;
                            $_SESSION['pengunjung'] = $data;
                            $_SESSION['id'] = $data->id_pengunjung;
                            $id_user = $_SESSION['id'];
                            // $_SESSION['email'] = $email;
                            // echo "<script>alert('Anda berhasil melakukan registrasi');</script>" ;
                            echo "<script>window.location='homepage-user.php?id=$id_user'</script>";
                        } else {
            ...
            <div class="login">
                <p class="have-account">Don’t have an account? <a class="have-account-login" href="signup.php">Sign up</a></p>
            </div>
        </div>
