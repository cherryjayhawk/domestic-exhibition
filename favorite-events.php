...
$result = mysqli_query($conn, "SELECT * FROM pengunjung WHERE id_pengunjung='".$_GET['id']."'");

$perPage = 6;
$page = isset($_GET['page']) ? (int) $_GET['page'] : 1;
$start = ($page > 1) ? ($page * $perPage) - $perPage : 0;

// $cards = "SELECT * from wishlist ORDER BY id_wishlist DESC LIMIT $start, $perPage";
$cards = "SELECT pengunjung.id_pengunjung, wishlist.id_wishlist, event.id_event, event.judul, event.poster FROM wishlist JOIN pengunjung USING (id_pengunjung) JOIN event USING (id_event) WHERE id_pengunjung=$id ORDER BY wishlist.id_wishlist DESC";
$all2 = mysqli_query($conn,$cards);

$all = mysqli_query($conn,"SELECT * FROM wishlist WHERE id_pengunjung=$id LIMIT 27");
$total = mysqli_num_rows($all);

$pages = ceil($total/$perPage);
// $sql = mysqli_query($conn, "SELECT * FROM wishlist JOIN ");
...
<body>
    ...
    <div class="container">
        ...
        <div class="container-items" style="background-color: white; border-left: 5px solid #BD622A;">
            ...
            <div class="favorite">
                <?php
                    // $sql = mysqli_query($conn, "SELECT user.id_user, simpan.id_simpan, resep.id_resep , user.nama_user, resep.judul_resep, resep.foto_menu, resep.viewers FROM simpan JOIN user USING (id_user) JOIN resep USING (id_resep) WHERE user.id_user = $id;");
                    // $card = $conn->query($sql);

                    // $result = mysqli_query($conn, "SELECT pengunjung.id_pengunjung, wishlist.id_wishlist, event.id_event, event.judul, event.poster FROM wishlist JOIN pengunjung USING (id_pengunjung) JOIN event USING (id_event) WHERE id_pengunjung=$id ORDER BY wishlist.id_wishlist DESC");
                ...
                <div class="favorite-items">
                        ...
                        <a href="unlist.php?id_wishlist=<?= $data['id_wishlist']; ?>&id_pengunjung=<?= $data['id_pengunjung']; ?>" class="text">
                            <svg width="32" height="28" viewBox="0 0 32 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16 6.05404C12.875 -1.28101 1.9375 -0.499763 1.9375 8.87528C1.9375 18.2503 16 26.0627 16 26.0627C16 26.0627 30.0625 18.2503 30.0625 8.87528C30.0625 -0.499763 19.125 -1.28101 16 6.05404Z" fill="#BD622A" stroke="#BD622A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                            </svg>
                        </a>
                ...
                <!-- <div class="favorite-items">
                    <div class="favorite-items-content">
                        <div class="favorite-img">
                            <img src="indofood.png" alt="">
                        </div>
                        <div class="favorite-name">
                            <svg width="32" height="28" viewBox="0 0 32 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M16 6.05404C12.875 -1.28101 1.9375 -0.499763 1.9375 8.87528C1.9375 18.2503 16 26.0627 16 26.0627C16 26.0627 30.0625 18.2503 30.0625 8.87528C30.0625 -0.499763 19.125 -1.28101 16 6.05404Z" fill="#BD622A" stroke="#BD622A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
        ...
        <div class="container-page">
            <?php
            for ($i = 1; $i <= $pages; $i++) { ?>
            <a class="page" href="?id=$id&page=<?php echo $i?>"><?php echo $i?></a>
            <?php } ?>
        </div>
    </div>
</body>
