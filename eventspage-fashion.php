...
$cards = "SELECT *, DATE_FORMAT(event_start_date, '%d %M %Y') as event_start_date_fix, DATE_FORMAT(event_end_date, '%d %M %Y') as event_end_date_fix, TIME_FORMAT(event_start_time, '%H.%i') as event_start_time_fix, TIME_FORMAT(event_end_time, '%H.%i WIB') as event_end_time_fix from event CROSS JOIN ticket ON event.id_event=ticket.id_event  WHERE kategori='Fashion' ORDER BY event.id_event DESC LIMIT $start, $perPage";
$all2 = mysqli_query($conn,$cards);

$all = mysqli_query($conn,"SELECT * FROM event WHERE kategori='Fashion' LIMIT 27");
